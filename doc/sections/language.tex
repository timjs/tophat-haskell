...
calculus \TOPHAT. First we define our host language, task and type grammer, together
with the static semantics. Then we describe the workings of each construct,
using examples. These construct will be formalized in Section~\ref{sec:semantics}.
...
Next to sequential composition,
a main pattern in workflow design is splitting up work in two or more tasks
that can be worked on simultaneously.
Both branches can progress over time by users' input.
This input needs to be addressed to the first or second task by tagging.
One could wait for all tasks to produce results and combine those,
or pick the first possible result available.
Both ways of parallelisation introduce explicit forks and implicit joins in our language.



\paragraph{Combination $(e_1 \And e_2)$}

A combination of two tasks,
like $\Edit 37 \And \Enter \String$,
is a parallel-and.
It waits for both branches to produce a result.
Only then it can pair up both results as its own.
...
\end{TASK}
will only step to its continuation when both the first, as well as the second editor are valued.
In that case, the internal step uses the paired integer and string to calculate the exclamation.
