...
Normalisation first ensure the expression is evaluated by the host language ($\evaluate$).
Then it will perform internal steps and internal choices by delegating this work to the stride semantics ($\stride$).
% A stride from task $t$ in state $s$ to $t'$ in state $s'$ is denoted by $\RelationS$.
These actions are repeated by \refrule{N-Repeat} till the resulting state and task stabilise.
