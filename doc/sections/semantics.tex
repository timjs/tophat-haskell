...
% The interact relation $\RelationI$ simply passes the event $i$ to the handle semantics
% and normalises the task afterwards.
% \refrule{I-Handle}
% Note that again,
% the double arrow $\drive{i}$ uses the single arrow $\handle{i}$.

Performing external steps and choices, and changing the values of editors is specified by the outermost layer of our semantics.
These interaction semantics are given in \cref{fig:interaction-semantics}.
To apply this semantics, we demand $t$ to be normalised.
Sending an input event $i$ to a task $t$ in state $s$ will first handle the event, and then prepare the resulting task for the next input by normalising it again.
...
Handling an input is done by a separate handling semantics, shown in \cref{fig:handling-semantics}.
This small step relation takes a task $t$ in a state $s$ and an input $i$ which results in a new task $t'$ in a new state $s'$.
Both interaction and handling are labeled transition systems which abstract away from a user interface.
Labels $i$ are formed from the grammar in \cref{fig:input-grammar}.
Input events live on the semantic level only and they are not accessible from within \TOPHAT.
