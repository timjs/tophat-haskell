...
\end{equation*}
Rule \refrule{N-Edit} uses the big step semantics from our base language ($\evaluate$)
to evaluate the wrapped expression $e$ to a value $v$.
Rule \refrule{N-Fill} has nothing to evaluate and just keeps the unvalued editor as is.
...
\begin{itemize}
  \item
    ...
    They are values with respect to evaluation of our base language, using the relation $\evaluate$.
  \item
    Tasks $t$ are a syntactical subset of pretasks $p$.
    They are values with respect to normalisation, using the relation $\normalise$.
\end{itemize}
