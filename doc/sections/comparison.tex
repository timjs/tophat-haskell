...
For example, the mutual exclusion problem for two parallel processes $P = a_p \to b_p \to P $ and $Q = a_q \to b_q \to Q$ can be stated as:
...
In $(\textit{inc} \Next \textit{dec}) \And (\textit{inc} \Next \textit{dec})$, the shared counter should never be greater than 1.
...
The task $t \Then c$ can only continue execution when two conditions are met:
