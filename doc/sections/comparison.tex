...
Second, we focus on features common to \TOP and \CSP, and study their respective realisation.
...
The central objective of \TOP is to coordinate the collaboration between people
...
It blocks evaluation of programs until the environment sends or receives events.
In both languages, communication \emph{between subsystems} does not block.
It evaluates programs as far as possible until further external input is required.
...
The step combinator in $t \Then e$ passes the value of $t$ to the continuation $e$.
...
\todo{These two paragraphs could be shorter --TS}
\CSP uses prefixing for both internal and external communication.
Parallel processes that all eventually are willing to engage in some action must wait for the others and then progress in one synchronised step.
...
\todo{Maybe remove this subsection? --TS}
...
The task $t \Then e$ acts like $t$ as long as the step is guarded, as described in \cref{sub:steps}.
...
instead of multiple processors that evaluate one or more programs.
\todo{Is this a correct statement? --TS}
...
In \TS{(inc x >>? dec) <&> (dec x >>? inc)}, the shared counter should at no point be greater than 1.
...
If some action $a$ is in the alphabets of both $P$ and $Q$, then their parallel composition $P \parallel Q$ can take an $a$-step only if both $P$ and $Q$ are ready to take an $a$-step, in which case they both take their $a$-step simultaneously.
...
Let \TS{match}, \TS{paper}, and \TS{tobacco} be shared Booleans.
