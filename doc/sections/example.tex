...
Note that every instance of the \TS{bookFlight} task has access to the same shared list of free seats.
...
but it would also be impossible to model the parallel execution of three \TS{bookFlight} tasks.
We do not know upfront which branch will finish first,
and thus we do not know how to thread the free seat list between the parallel tasks.
...
\begin{figure}[h]
  ...
  \caption{
    ...
    Also, the message bubble shows only integer values are allowed in the \TS{age} field.
  }
