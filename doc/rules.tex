...
\newrule{H-Empty}[v : \beta]
  { }
  {\Edit v,s \handle{\Empty} \Enter \beta,s}

\newrule{H-Fill}[v' : \beta]
  { }
  {\Enter \beta,s \handle{v'} \Edit v',s}

\newrule{H-Update}[s(l), v' : \beta]
  { }
...
\newrule{H-Next}[\Value{(t_1,s)}\neq \bot \wedge \neg\Failing{(e_2 \Value{(t_1,s)}\normalise)}]
  { }
  {t_1 \Next e_2,s \handle{\Continue} t_1 \Then e_2,s}
...
\newrule{H-PickLeft}[\Failing(t_1)]
  {t_1,s \handle{\Pick r} t_1',s'}
  {t_1 \Xor t_2,s \handle{\Pick \Left r} t_1',s'}

\newrule{H-PickRight}[\Failing(t_2)]
  {t_2,s \handle{\Pick r} t_2',s'}
  {t_1 \Xor t_2,s \handle{\Pick \Right r} t_2',s'}

\newrule{H-PickHere}
  {\ }
  {t,s \handle{\Pick \Here} t,s}
