...
\newmacro{st}[2][1]
  {\ifthenelse{\isempty{#1}}
    {\ifstateful{\ \shortmid \mu#2}\else{}\fi}
    {\ifstateful{\ \shortmid [#1]\mu#2}\else{}\fi}}
  % {\ifstateful{, [#1]\mu#2}\else{}\fi}
\newmacro{St}[1]
  {\ifstateful{\ \shortmid \Sigma#1}\else{}\fi}
...
\newrule{H-Fail}
  { }
  {\Fail\st{} \handle{h} \Fail\st{}}


\newrule{H-PassThen}
  {t_1\st{} \handle{h} t_1'\st{'}}
  {t_1 \Then e_2\st{} \handle{h} t_1' \Then e_2\st{'}}

\newrule{H-PassNext}
  {t_1\st{} \handle{h} t_1'\st{'}}
  {t_1 \Next e_2\st{} \handle{h \neq \Continue} t_1' \Next e_2\st{'}}
...
\newrule{H-FirstAnd}
  {t_1\st{} \handle{h} t_1'\st{'} }
  {t_1 \And t_2\st{} \handle{\First h} t_1' \And t_2\st{'}}

\newrule{H-SecondAnd}
  {t_2\st{} \handle{h} t_2'\st{'} }
  {t_1\st{} \And t_2 \handle{\Second h} t_1 \And t_2'\st{'}}


\newrule{H-FirstOr}
  {t_1\st{} \handle{h} t_1'\st{'} }
  {t_1 \Or t_2\st{} \handle{\First h} t_1' \Or t_2\st{'}}

\newrule{H-SecondOr}
  {t_2\st{} \handle{h} t_2'\st{'} }
  {t_1\st{} \Or t_2 \handle{\Second h} t_1 \Or t_2'\st{'}}
...
% \newrule{H-PassS}
%   {t_1\st{} \handle{h} t_1'\st{'}}
%   {t_1 \Next e\st{} \handle{h} t_1' \Next e\st{'}}

% \newrule{H-Pass}[h \neq \Execute \pi]
%   {t_1\st{} \handle{h} t_1'\st{'}       \\
%    t_1' \Then e\st{'} \normalise t_2\st{''} }
%   {t_1 \Then e\st{} \handle{h} t_2\st{''}}
